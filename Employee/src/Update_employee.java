import java.sql.*;
import javax.swing.*;
import java.awt.*;
import java.awt.event.*;

public class Update_employee extends JFrame implements ActionListener,ItemListener{
    JLabel l1,l2,l3,l4,l5,l6,l7,l8,emp;
    JTextField t1,t3,t4,t5,t6,t7,t8;
    JButton b1,b2;
    Choice ch1,ch2;

    Update_employee(){

        super("Update Employee");

        setLayout(null);
        getContentPane().setBackground(Color.WHITE);

        ch2 = new Choice();
        ch2.setBounds(160,40,200,20);

        try{
            conn c = new conn();
            ResultSet rs = c.s.executeQuery("select * from employee");

            while(rs.next()){
                ch2.add(rs.getString("id"));
            }
        }catch(Exception e){ }

        emp = new JLabel("Empno id");
        emp.setBounds(40,40,100,20);
        add(emp);
        add(ch2);

        l1 = new JLabel("Name : ");
        t1 = new JTextField(15);

        l1.setBounds(40,80,100,20);
        t1.setBounds(160,80,200,20);
        add(l1);
        add(t1);

        ch1 = new Choice();
        ch1.add("Male");
        ch1.add("Female");

        l2 = new JLabel("Gender : ");

        l2.setBounds(40,120,100,20);
        ch1.setBounds(160,120,200,20);
        add(l2);
        add(ch1);

        l3 = new JLabel("Address : ");
        t3 = new JTextField(15);

        l3.setBounds(40,160,100,20);
        t3.setBounds(160,160,200,20);
        add(l3);
        add(t3);

        l4 = new JLabel("Age : ");
        t4 = new JTextField(15);

        l4.setBounds(40,200,100,20);
        t4.setBounds(160,200,200,20);
        add(l4);
        add(t4);

        l5 = new JLabel("DOB : ");
        t5 = new JTextField(15);

        l5.setBounds(40,240,100,20);
        t5.setBounds(160,240,200,20);
        add(l5);
        add(t5);

        l6 = new JLabel("Post : ");
        t6 = new JTextField(15);

        l6.setBounds(40,280,100,20);
        t6.setBounds(160,280,200,20);
        add(l6);
        add(t6);

        l7 = new JLabel("Email: ");
        t7= new JTextField(15);

        l7.setBounds(40,320,100,20);
        t7.setBounds(160,320,200,20);
        add(l7);
        add(t7);

        l8 = new JLabel("Phone : ");
        t8= new JTextField(15);

        l8.setBounds(40,320,100,20);
        t8.setBounds(160,320,200,20);
        add(l8);
        add(t8);

        b1 =new JButton("Update");
        b2 = new JButton("Delete");

        b1.setBounds(40,400,150,30);
        b2.setBounds(200,400,150,30);
        add(b1);
        add(b2);

        b1.setBackground(Color.BLACK);
        b1.setForeground(Color.WHITE);

        b2.setBackground(Color.BLACK);
        b2.setForeground(Color.WHITE);

        b1.addActionListener(this);
        b2.addActionListener(this);

        ch2.addItemListener(this);

        setVisible(true);
        setSize(400,550);
        setLocation(600,200);
    }

    public void actionPerformed(ActionEvent ae){

        if(ae.getSource()==b1){
            String n = t1.getText();
            String g = ch1.getSelectedItem();
            String a = t3.getText();
            String age = t4.getText();
            String dob = t5.getText();
            String post = t6.getText();
            String e = t7.getText();
            String ph = t8.getText();

            String qry = "update employee set name='"+n+"',gender='"+g+"',address='"+a+"',dob='"+dob+"',post='"+post+"',email='"+e+"',phone='"+ph+"'   where id="+ch2.getSelectedItem();

            try{
                conn c1 = new conn();
                c1.s.executeUpdate(qry);
                JOptionPane.showMessageDialog(null,"Employee Updated");
                this.setVisible(false);
                new details();
            }catch(Exception ee){
                ee.printStackTrace();
            }
        }

        if(ae.getSource()==b2){
            try{
                conn c1 = new conn();
                c1.s.executeUpdate("delete from employee where id="+ch2.getSelectedItem());
                JOptionPane.showMessageDialog(null,"Employee Deleted");
                this.setVisible(false);
                new details();
            }catch(Exception ee){
                ee.printStackTrace();
            }
        }
    }

    public void itemStateChanged(ItemEvent ie){
        try{
            conn c1 = new conn();
            ResultSet rs = c1.s.executeQuery("select * from employee where id="+ch2.getSelectedItem());

            if(rs.next()){
                t1.setText(rs.getString("name"));
                t3.setText(rs.getString("address"));
                t4.setText(rs.getString("age"));
                t5.setText(rs.getString("dob"));
                t6.setText(rs.getString("post"));
                t7.setText(rs.getString("email"));
                t8.setText(rs.getString("phone"));
            }
        }catch(Exception ee){
            ee.printStackTrace();
        }

    }

    public static void main(String[] args){
        new Update_employee();
    }

}
